...
export function replaceRefs(value, schemaMap, responseMap, paramMap) {
...
export function deepReplaceRefs(node, schemaMap, responseMap, paramMap) {
  if (node === null || node === undefined) return;
  if (typeof node === 'string') {
    return replaceRefs(node, schemaMap, responseMap, paramMap);
  }
  if (Array.isArray(node)) {
    for (let i = 0; i < node.length; i++) {
      ...
      node[i] =
        typeof v === 'string'
          ? replaceRefs(v, schemaMap, responseMap, paramMap)
          : (deepReplaceRefs(v, schemaMap, responseMap, paramMap), v);
    }
    return;
  }
  if (typeof node === 'object') {
    for (const k of Object.keys(node)) {
      const v = node[k];
      if (typeof v === 'string') {
        node[k] = replaceRefs(v, schemaMap, responseMap, paramMap);
      } else {
        deepReplaceRefs(v, schemaMap, responseMap, paramMap);
      }
    }
  }
}
